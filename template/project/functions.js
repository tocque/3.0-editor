var functions_d6ad677b_427a_4623_b50f_a445a3b0ef8a = 
{
	...
	core.updateEnemys();
        ...
        "win": function win(reason, norank) {
	// 游戏获胜事件 
	core.ui.closePanel();
	...
	core.waitHeroToStop(() => {
        ...
        "changingFloor": function changingFloor(floorId, heroLoc, fromLoad) {
	...
	if (core.status.maps[floorId].bgm) {
		...
		if (Array.isArray(bgm)) bgm = bgm[0];
	...
	// ...可以新增一些其他内容，比如创建个画布在右上角显示什么内容等等
        ...
        "afterChangeFloor": function afterChangeFloor(floorId, fromLoad) {
	...
	return true;
        ...
        "beforeBattle": function beforeBattle(enemyId, x, y) {
	...
	return true;
        ...
        "afterBattle": function afterBattle(enemyId, x, y, callback) {
	// 战斗结束后触发的事件
	...
	*/
	...
	if (callback) callback();
        ...
        "afterOpenDoor": function afterOpenDoor(doorId, x, y, callback) {
	// 开一个门后触发的事件
	...
	if (callback) callback();
        ...
        "afterGetItem": function afterGetItem(itemId, x, y, callback) {
	// 获得一个道具后触发的事件
	...
	if (callback) callback();
        ...
        "afterChangeLight": function afterChangeLight(x,y) {
	// 改变亮灯之后，可以触发的事件

},
        "afterPushBox": function afterPushBox() {
	// 推箱子后的事件
	if (core.searchBlock('box').length == 0) {
		...
		*/
        ...
        "afterUseBomb": function afterUseBomb() {
	// 使用炸弹/圣锤后的事件
	...
	*/
        ...
        "afterPassNet": function afterPassNet(x, y, id) {
	...
	// if (id == 'lavaNet') core.removeBlock(x, y);
        ...
        "canUseQuickShop": function canUseQuickShop(shopId) {
	// 当前能否使用某个快捷商店
	// shopId：快捷商店ID
	...
	return [
		...
		[10, "模仿", "怪物的攻防和勇士攻防相等"],
		[11, "吸血", ([ratio, add]) => `战斗前，怪物首先吸取角色的${Math.floor(100 * ratio || 0)}%生命（约${Math.floor((ratio || 0) * core.getStatus('hp'))}点）作为伤害${add ? "，并把伤害数值加到自身生命上" : ""}`],
        ...
        "getEnemyInfo": function getEnemyInfo(enemy, hero, x, y, floorId) {
	...
	return {
        ...
        "getDamageInfo": function getDamageInfo(enemy, hero, x, y, floorId) {
	// 获得战斗伤害信息（实际伤害计算函数）
	// 
	// 参数说明：
	// enemy：该怪物信息
	...
	return {
        ...
        "updateEnemys": function updateEnemys() {
	...
	*/
    ...
    "actions": {
        "onKeyUp": function onKeyUp(keyCode, altKey) {
	...
	*/
        ...
        "onStatusBarClick": function onStatusBarClick() {
	...
	core.status.checkBlock = {
        ...
        "beforeMoveStep": function beforeMoveStep(canmove) {
		
	},
        "moveOneStep": function moveOneStep(x, y) {
	...
	// core.updateStatusBar();
        ...
        "moveDirectly": function moveDirectly(x, y, ignoreSteps) {
	...
	return false;
        ...
        "parallelDo": function parallelDo(timestamp) {
	...
	*/
    ...
    "ui": {
        "drawStatusBar": function drawStatusBar() {
	...
	ctx.fillText(core.statusBar.redKey.innerText, leftOffset + 75, topOffset + 20);
        ...
        "drawStatistics": function drawStatistics() {
	// 浏览地图时参与的统计项目
