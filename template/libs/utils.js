...
utils.prototype.unshift = function (a, b) {
    if (!Array.isArray(a) || b == null) return;
    if (Array.isArray(b)) {
...
utils.prototype.inArray = function (array, element) {
    return Array.isArray(array) && array.indexOf(element) >= 0;
}
...
utils.prototype.unzip = async function (blobOrUrl, convertToText) {

    if (!window.zip) {
        throw new Error("zip.js not exists!");
    }

    if (typeof blobOrUrl == 'string') {
        blobOrUrl = await axios.get(blobOrUrl, {
            responseType: 'blob'
        }).then((res) => new Blob([res.data], 'application/zip'));
    }

    if (!(blobOrUrl instanceof Blob)) {
        throw new Error("Should use Blob or URL as input");
    }

    return new Promise((res, rej) => {
        zip.createReader(new zip.BlobReader(blobOrUrl), res, rej);
    }).then((reader) => new Promise((res, rej) => {
        reader.getEntries(async (entries) => {
            const data = await core.utils._unzip_readEntries(entries, convertToText);
            reader.close(() => res(data));
        });
    }));
}

utils.prototype._unzip_readEntries = async function (entries, convertToText) {
    const results = {};
    if (entries == null) {
        return results;
    }
    const tasks = entries.forEach((entry) => new Promise((res, rej) => {
        entry.getData(convertToText ? new zip.TextWriter('utf8') 
            : new zip.BlobWriter(), (data) => {
