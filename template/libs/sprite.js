...
sprite.prototype._init = function(){
    ...
    /** 各类型的默认定义  */
    this.defaultTypes = {
        'items': { height: 32, width: 32, frame: 1, custom: true },
        'tilesets': { height: 32, width: 32, frame: 1 },
        'enemys': { height: 32, frame: 4, row: 1, custom: true },
        'autotiles': { height: 128, width: 96, row: 1, single: true },
        'events': { height: 32, frame: 4, row: 1, custom: true },
        // '32,32,1':'terrains',
        // '128,32,4':'animates',
        // '64,32,2':'npcs',
        // '128,48,4':'npc48',
...
sprite.prototype._generateTextures = function() {
    ...
    const baseTextures = this.baseTextures || {};
    this.baseTextures = baseTextures;
    for (let dir in this.defaultTypes) {
        const defaultType = this.defaultTypes[dir];
        if (!defaultType.custom) {
            for (let i in core.images[dir]) {
                baseTextures[i] = Texture.from(core.images[dir][i]);
                this._generateTypedTexture(i, baseTextures[i], defaultType, core.metadata[dir]);
            }
        } else {
            for (let type in core.images[dir]) {
                const param = this._calParam(defaultType, type);
                for (let i in core.images[dir][type]) {
                    baseTextures[i] = Texture.from(core.images[dir][type][i]);
                    this._generateTypedTexture(i, baseTextures[i], param, core.metadata[dir][type]);
                }
            }
        }
    }
...
sprite.prototype._calParam = function(defaultType, typestr) {
    if (typestr == 'default') {
        return defaultType;
    } else if (typestr == 'sprites') {
        return { frame: 4, line: 4, row: 1, single: true }
    } else {
        const param = Object.assign({}, defaultType);
        typestr.replace(/(@|\$)?([\[\]\d]+)/g, (s, idf, v) => {
            if (idf == '@') {
                param.frame = parseInt(v);
            } else if (idf == '$') {
                if (v[0] == '[') {
                    param.width = parseInt(v.slice(1, -1));
                    delete param.row;
                } else {
                    param.row = parseInt(v);
                    delete param.width;
                }
            } else {
                if (v[0] == '[') {
                    param.height = parseInt(v.slice(1, -1));
                    delete param.line;
                } else {
...
sprite.prototype._calSize = function(img, { width, height, row, line, frame }) {
    if (frame) {
        if (width) {
            row = img.width / width / frame;
        } else {
            width = img.width / row / frame;
        }
    } else {
        frame = img.width / row / width;
    }
    if (height) {
        line = img.height / height;
    } else {
        height = img.height / line;
    }
    return {
        width, height, row, line, frame
    }
}

sprite.prototype._getImages = function(self, dir) {
    const defaultType = self.defaultTypes[dir];
    const images = [];
    if (!defaultType.custom) {
        for (let i in core.images[dir]) {
            const img = core.images[dir][i];
            images.push({
                img,
                meta: {
                    ...self._calSize(img, defaultType), blocks: []
                }
            })
        }
    } else {
        for (let type in core.images[dir]) {
            const param = self._calParam(defaultType, type);
            for (let i in core.images[dir][type]) {
                const img = core.images[dir][type][i];
                images.push({
                    img,
                    meta: {
                        ...self._calSize(img, param), blocks: []
                    }
                })
            }
        }
...
/**
 * 生成纹理
 * 注意到it是引用, 不能直接改动, 必须进行解构
 */
sprite.prototype._generateTypedTexture = function(id, texture, it, metadata) {
    const tmp = [];
    const { width, height, row, line, frame: nf } = this._calSize(texture.orig, it);
    if (it.single) {
        const temp = new Array(line);
        for(let i = 0; i < line; i++) {
            temp[i] = [];
            for (let j = 0; j < nf; j++) {
                try {
                    temp[i].push(new Texture(
                        texture, new Rectangle(j*width, i*height, width, height)
                    ));
                }
                catch(e){
                    debugger;
        ...
        this.textures[id] = temp;
        return;
    }
    const cnt = line * row;
    const metatable = {};
    for (let i in metadata[id]) {
        metatable[metadata[id][i]] = i;
    }
    for(let i = 0; i < cnt; i++) {
        if (!metatable[i]) continue;
        const temp = [], 
            nx = (i % row) * width, 
            ny = Math.floor(i / row) * height;
        for (let j = 0; j < nf; j++) {
            try {
                temp.push(new Texture(
                    texture, new Rectangle(nx+j*width, ny, width, height)
                ));
            }
            catch(e){
                debugger;
        ...
        this.textures[metatable[i]] = [temp];
    }
}
...
canvasLayer.prototype.updateTexture = function(canvas){
    if(canvas)this.texture = Texture.from(canvas);
    this.texture.update();
}
