...
export const addUsedFlag = function() {
    s.replace(/flag:([a-zA-Z0-9_\u4E00-\u9FCC]+)/g, (s0, s1) => {
        used_flags[s1] = true; return s0;
    });
    s.replace(/flags\.([a-zA-Z_]\w*)/g, (s0, s1) => {
        used_flags[s1] = true; return s0;
    });
    if (game.runtime.flags) {
        for (var s in used_flags) {
            if (!(s in game.runtime.flags)) {
                game.runtime.flags[s] = null;
            }
        }
    }
}